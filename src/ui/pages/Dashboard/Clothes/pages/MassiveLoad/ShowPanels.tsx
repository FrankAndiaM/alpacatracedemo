...
import { paginateIndividualRegisterPanelsTransaction } from '~services/massive_load_panels';

type ShowMassiveLoadPanelsProps = any;
...
const ShowMassiveLoadPanels: React.FC<ShowMassiveLoadPanelsProps> = () => {
  const {
    auth: {
  ...
  const _paginateIndividualRegisterTransaction = useCallback(
    (page: number, per_page: number, sort_by: string, order: string, search: string) => {
      return paginateIndividualRegisterPanelsTransaction(
        file_loaded_id ?? '',
        page,
        per_page,
        sort_by,
            ...
            <Box>
              <Breadcrumbs
                breadcrumbs={[
                  ...
                  // },
                  {
                    path: `${routes.clothesFileListLoaded}#3`,
                    // onClick: () => {
                    //   history(-1);
                    // },
...
export default ShowMassiveLoadPanels;
