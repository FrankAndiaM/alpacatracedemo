...
type UploadMassiveLoadYarnsDialogProps = {
  open: boolean;
  closeAction: any;
};

const UploadMassiveLoadYarnsDialog: React.FC<UploadMassiveLoadYarnsDialogProps> = (
  props: UploadMassiveLoadYarnsDialogProps
) => {
  const {
    auth: {
      organizationTheme: { organizationId, attributes_relation }
    }
  ...
  const handleDownloadExcel = useCallback(() => {
    const arrTypesHeader = ['(texto)', '(texto)', '(texto)', '(texto)', '(Foto)', ...additionalTypesHeaders];
    const headers = [
      'Código de hilo - Trazabilidad',
      'Material del hilo',
      'Código de Color',
      'Título del hilo',
      'Foto del hilo',
    ...
    downloadExcel('Plantilla', 'plantilla_hilos.xlsx', [arrTypesHeader, headers], colsWch);
    // saveAs(COMMUNITY_BASE_URL_S3 + 'Formulario de Prendas.xlsx');
  }, [additionalSchemaHeaders, additionalTypesHeaders]);
      ...
      .then((res: any) => {
        ...
        if (schemaAttr) {
          ...
          const newSchema: string[] = schemaAttr
            .filter((element: any) => {
              if (element.id && ['Material del hilo', 'Código de Color', 'Título del hilo'].includes(element?.name)) {
                return false;
              }
  ...
  useEffect(() => {
    if (attributes_relation && Array.isArray(attributes_relation)) {
      const relation = attributes_relation.find((element: any) => element?.entity_model_type === 'Yarns');
        ...
        <Grid container={true}>
          ...
          <Grid item={true} xs={10} sm={10} md={10} lg={10} xl={10}>
            <Typography color="#2F3336" fontWeight={600}>
              REGISTRA LOS DATOS DE LOS HILOS DE FORMA MASIVA
            </Typography>
            <Typography color="#959595" my={1}>
              Descarga la plantilla para registrar tus hilos.
            </Typography>
            {isLoadingForm ? (
        ...
        <Grid container={true} mt={3}>
          ...
          <Grid item={true} xs={10} sm={10} md={10} lg={10} xl={10}>
            <Typography color="#2F3336" fontWeight={600} lineHeight="24px">
              SUBE EL ARCHIVO CON TUS HILOS
            </Typography>
            <Typography color="#959595">
              Sigue las instrucciones que encontrarás en el Excel para subir tus hilos sin problema.
            </Typography>
...
export default UploadMassiveLoadYarnsDialog;
