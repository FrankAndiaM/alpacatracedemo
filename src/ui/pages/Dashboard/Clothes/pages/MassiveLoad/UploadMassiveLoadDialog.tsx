...
type UploadMassiveLoadDialogProps = {
  open: boolean;
  closeAction: any;
};

const UploadMassiveLoadDialog: React.FC<UploadMassiveLoadDialogProps> = (props: UploadMassiveLoadDialogProps) => {
  const {
    auth: {
      organizationTheme: { organizationId, attributes_relation, show_product }
    }
    ...
    (file: File) => {
      ...
      ConvertExcelFileToCSV(file)
        .then((convertedFile: any) => {
          ...
          formData.append('entity', 'Clothes');
  ...
  const handleDownloadExcel = useCallback(() => {
    const arrTypesHeader = [
      '(texto)',
      '(texto)',
      '(2023-11-15)',
      show_product && '(PA78SS564, PA447879)',
      '(UE5ADSDSA, 45AWWEDF)',
      '(Foto)',
      ...additionalTypesHeaders
    ];
    const headers = [
      'Código de prenda - Trazabilidad',
      'Descripción',
      'Fecha de producción',
      show_product && 'Paneles',
      'Hilos',
      'Foto de la prenda',
    ...
    downloadExcel('Plantilla', 'plantilla_prendas.xlsx', [arrTypesHeader, headers], colsWch);
    // saveAs(COMMUNITY_BASE_URL_S3 + 'Formulario de Prendas.xlsx');
  }, [additionalSchemaHeaders, additionalTypesHeaders, show_product]);
  ...
  const handleValidateHeaders = useCallback(
    ...
    [additionalSchemaHeaders, additionalTypesHeaders, isLoadingForm, show_product]
  );
  useEffect(() => {
    if (attributes_relation && Array.isArray(attributes_relation)) {
      const relation = attributes_relation.find((element: any) => element?.entity_model_type === 'Clothes');
        ...
        <Grid container={true}>
          ...
          <Grid item={true} xs={10} sm={10} md={10} lg={10} xl={10}>
            <Typography color="#2F3336" fontWeight={600}>
              REGISTRA LOS DATOS DE LAS PRENDAS DE FORMA MASIVA
            </Typography>
            <Typography color="#959595" my={1}>
              Descarga la plantilla para registrar tus prendas.
            </Typography>
            {isLoadingForm ? (
        ...
        <Grid container={true} mt={3}>
          ...
          <Grid item={true} xs={10} sm={10} md={10} lg={10} xl={10}>
            <Typography color="#2F3336" fontWeight={600} lineHeight="24px">
              SUBE EL ARCHIVO CON TUS PRENDAS
            </Typography>
            <Typography color="#959595">
              Sigue las instrucciones que encontrarás en el Excel para subir tus prendas sin problema.
            </Typography>
...
export default UploadMassiveLoadDialog;
