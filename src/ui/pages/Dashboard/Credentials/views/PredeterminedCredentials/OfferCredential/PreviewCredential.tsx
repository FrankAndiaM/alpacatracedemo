...
const useStyles: any = makeStyles((theme: Theme) => ({
  ...
  boxCard: {
    borderRadius: '6.762px 6.762px 7px 7px',
...
const PreviewCredential: React.FC<PreviewCredentialProps> = (props: PreviewCredentialProps) => {
  const { issuedCredential, organizationTheme, clothe, showProduct } = props;
  const classes = useStyles();
  const theme = useTheme();

  // const showStatus = useCallback((_status?: string) => {
  ...
  const renderArray = useCallback((value: any): string => {
    let str = '';
  ...
  return (
    <>
      <Box maxHeight={400} position={'relative'}>
        <Box className={classes.boxCard}>
          ...
          <Box className={classes.boxClaims}>
            ...
            <Box className={classes.claimsStyle} display="flex">
              <Box component="span">{'Nombre de prenda: '}&nbsp;</Box>
              <Box component="span">{clothe?.name ?? ''}</Box>
            </Box>
            <Box className={classes.claimsStyle} display="flex">
              <Box component="span">{'Código: '}&nbsp;</Box>
              <Box component="span">{clothe?.code ?? ''}</Box>
            </Box>
            <Box className={classes.claimsStyle} display="flex">
              <Box component="span">{'Fecha de producción: '}&nbsp;</Box>
              <Box component="span">{clothe?.production_at ?? ''}</Box>
            </Box>
            {showProduct && (
              <Box className={classes.claimsStyle} display="flex">
                <Box component="span">{'Panel(es): '}&nbsp;</Box>
                <Box component="span">{renderArray(clothe?.fabric_inventories ?? [])}</Box>
              </Box>
            )}
            <Box className={classes.claimsStyle} display="flex">
              <Box component="span">{'Hilo(s): '}&nbsp;</Box>
              <Box component="span">{renderArray(clothe?.yarns ?? [])}</Box>
            </Box>
            ...
            <Box px={2} py={1} fontSize={13} overflow={'auto'} zIndex={2}>
              <Box display="flex" justifyContent="space-between" my={1} flexDirection={{ xs: 'column', md: 'row' }}>
                <Box display="flex" alignItems="center">
                  <Box fontWeight={600}>Verificado por:&nbsp;</Box>
                  <Box ml={2}>
                ...
                {/* <Box maxWidth="220px" textAlign={'end'}>
                  <Box style={{ wordBreak: 'break-all' }}>
                    https://explorer.lacchain.net/address/0x8Ce6e464462195373A40aB2f175640fb0F691133
                  </Box>
                </Box> */}
              </Box>
            </Box>
          </Box>
          <Box style={{ position: 'absolute', right: '8px', zIndex: 1 }}>
            <Box>
